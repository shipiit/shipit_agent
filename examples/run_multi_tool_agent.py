...
def build_llm_from_env(provider: str | None = None):
    ...
    if selected == "anthropic":
        ...
        return AnthropicChatLLM(
            model=os.getenv("SHIPIT_ANTHROPIC_MODEL", "claude-3-5-sonnet-latest")
        )
    if selected == "gemini":
        _require_any(["GEMINI_API_KEY", "GOOGLE_API_KEY"], provider="gemini")
        return GeminiChatLLM(
            model=os.getenv("SHIPIT_GEMINI_MODEL", "gemini/gemini-1.5-pro")
        )
    if selected in {"vertex", "vertex_ai", "vertexai"}:
        ...
        credentials_file = os.getenv("SHIPIT_VERTEX_CREDENTIALS_FILE") or os.getenv(
            "GOOGLE_APPLICATION_CREDENTIALS"
        )
        project_id = os.getenv("VERTEXAI_PROJECT") or os.getenv("GOOGLE_CLOUD_PROJECT")
        location = (
    ...
    if selected == "groq":
        ...
        return GroqChatLLM(
            model=os.getenv("SHIPIT_GROQ_MODEL", "groq/llama-3.3-70b-versatile")
        )
