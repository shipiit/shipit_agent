...
class GoogleDriveTool(ConnectorToolBase):
    ...
    def __init__(
        self,
        *,
        credential_key: str = "google_drive",
        credential_store=None,
        name: str = "google_drive",
        description: str = "Search and list Google Drive files.",
        ...
        self.prompt_instructions = (
            "Use this for document discovery and Drive file lookup."
        )
    ...
    def _build_service(self, record):
        try:
            ...
            raise RuntimeError(
                "Install `google-api-python-client` and `google-auth` to use GoogleDriveTool."
            ) from exc
    ...
    def run(self, context: ToolContext, **kwargs: Any) -> ToolOutput:
        ...
        response = (
            service.files()
            .list(
                q=str(kwargs.get("query", "")).strip() or None,
                pageSize=int(kwargs.get("page_size", 10)),
        ...
        lines = [
            f"{item.get('name', '?')} | {item.get('mimeType', '?')} | {item.get('webViewLink', '')}"
            for item in items
        ]
        return ToolOutput(
            text="\n".join(lines) if lines else "No Google Drive files found.",
            metadata={
