...
class MemoryTool:
    def __init__(
        ...
        *,
        ...
        self.prompt_instructions = (
            "Use this to persist durable facts that may matter across turns, sessions, or workflows."
        )
    ...
    def run(self, context: ToolContext, **kwargs) -> ToolOutput:
        ...
        if action == "store":
            ...
            fact = MemoryFact(content=content, category=str(kwargs.get("category", "general")))
            memory_store.add(fact)
            return ToolOutput(text=f"Stored memory: {content}", metadata={"content": content, "category": fact.category})

        if action == "search":
            query = str(kwargs.get("query", "")).strip()
            results = memory_store.search(query)
            lines = [f"- {fact.content}" for fact in results]
            return ToolOutput(text="\n".join(lines) if lines else "No memories found.", metadata={"query": query, "count": len(results)})
