...
class DecisionMatrixTool:
    def __init__(
        ...
        *,
        ...
        self.prompt_instructions = (
            "Use this when there are multiple valid paths and the agent should recommend one clearly."
        )
            ...
            "function": {
                ...
                "parameters": {
                    ...
                    "properties": {
                        ...
                        "options": {"type": "array", "description": "Candidate options"},
    ...
    def run(self, context: ToolContext, **kwargs) -> ToolOutput:
        ...
        options = [str(item).strip() for item in kwargs.get("options", []) if str(item).strip()]
        criteria = [str(item).strip() for item in kwargs.get("criteria", []) if str(item).strip()]
        ...
        for option in options:
            criteria_summary = ", ".join(criteria) if criteria else "No criteria provided"
            lines.append(f"- {option}: evaluate against {criteria_summary}")
        lines.append(f"Recommendation: {recommendation}")
        lines.append(f"Fallback: {fallback}")
        lines.append("Tradeoffs:")
        lines.append("- Favor the option with the strongest fit to the highest-priority criteria.")
