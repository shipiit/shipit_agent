...
class JiraTool(HTTPConnectorToolBase):
    ...
    def __init__(
        self,
        *,
        credential_key: str = "jira",
        credential_store=None,
        name: str = "jira",
        description: str = "Search Jira issues and create Jira issues.",
            ...
            "function": {
                ...
                "parameters": {
                    ...
                    "properties": {
                        ...
                        "issue_key": {
                            "type": "string",
                            "description": "Jira issue key such as PROJ-123",
                        },
                        "transition_id": {"type": "string"},
                        "comment_body": {"type": "string"},
                        "account_id": {
                            "type": "string",
                            "description": "Jira account ID for assignment",
                        },
    ...
    def run(self, context: ToolContext, **kwargs: Any) -> ToolOutput:
        ...
        if action == "search_issues":
            data = self._request_json(
                ...
                body={
                    ...
                    "maxResults": int(kwargs.get("limit", 10)),
            ...
            return ToolOutput(
                text="\n".join(
                    f"{issue.get('key', '?')}: {issue.get('fields', {}).get('summary', '')} [{issue.get('fields', {}).get('status', {}).get('name', '')}]"
                    for issue in issues
                )
                if issues
                else "No Jira issues found.",
                metadata={
        ...
        if action == "create_issue":
            body = {
                "fields": {
                    "project": {"key": str(kwargs.get("project_key", ""))},
                    "summary": str(kwargs.get("summary", "")),
                    "description": str(kwargs.get("description_text", "")),
                    "issuetype": {"name": str(kwargs.get("issue_type", "Task"))},
                }
            }
            data = self._request_json(
                record=record, method="POST", path="/rest/api/3/issue", body=body
            )
            return ToolOutput(
                text=f"Jira issue created: {data.get('key', '?')}",
                metadata={
                    "provider": self.provider,
                    "connected": True,
                    "action": action,
        ...
        if action == "get_issue":
            ...
            if not issue_key:
                return ToolOutput(
                    text="issue_key is required for get_issue.",
            ...
            data = self._request_json(
                ...
                query={"fields": "summary,status,assignee,description"},
            )
        ...
        if action == "list_transitions":
            ...
            if not issue_key:
                return ToolOutput(
                    text="issue_key is required for list_transitions.",
                    metadata={"provider": self.provider, "action": action},
                )
            data = self._request_json(
                record=record,
            ...
            return ToolOutput(
                ...
                if transitions
                else f"No Jira transitions found for {issue_key}.",
                metadata={
                    "provider": self.provider,
                    "connected": True,
                    "action": action,
        ...
        if action == "transition_issue":
            ...
            if not issue_key or not transition_id:
                return ToolOutput(
                    text="issue_key and transition_id are required for transition_issue.",
                    metadata={"provider": self.provider, "action": action},
                )
            data = self._request_json(
                record=record,
            ...
            return ToolOutput(
                text=f"Jira issue transitioned: {issue_key} -> {transition_id}",
                metadata={
                    "provider": self.provider,
                    "connected": True,
                    "action": action,
        ...
        if action == "add_comment":
            ...
            if not issue_key or not comment_body:
                return ToolOutput(
                    text="issue_key and comment_body are required for add_comment.",
                    metadata={"provider": self.provider, "action": action},
                )
            body = {
                "body": {
            ...
            return ToolOutput(
                text=f"Comment added to Jira issue {issue_key}.",
                metadata={
                    "provider": self.provider,
                    "connected": True,
                    "action": action,
        ...
        if action == "assign_issue":
            ...
            if not issue_key or not account_id:
                return ToolOutput(
                    text="issue_key and account_id are required for assign_issue.",
                    metadata={"provider": self.provider, "action": action},
                )
            data = self._request_json(
                record=record,
            ...
            return ToolOutput(
                text=f"Jira issue assigned: {issue_key} -> {account_id}",
                metadata={
                    "provider": self.provider,
                    "connected": True,
                    "action": action,
