...
class GoogleCalendarTool(ConnectorToolBase):
    ...
    def __init__(
        self,
        *,
        credential_key: str = "google_calendar",
        credential_store=None,
        name: str = "google_calendar",
        description: str = "Search and list Google Calendar events.",
    ...
    def _build_service(self, record):
        try:
            ...
            raise RuntimeError(
                "Install `google-api-python-client` and `google-auth` to use GoogleCalendarTool."
            ) from exc
    ...
    def run(self, context: ToolContext, **kwargs: Any) -> ToolOutput:
        ...
        lines = [
            f"{item.get('summary', '(No Title)')} | {item.get('start', {}).get('dateTime') or item.get('start', {}).get('date')} | {item.get('htmlLink', '')}"
            for item in items
        ]
        return ToolOutput(
            text="\n".join(lines) if lines else "No Google Calendar events found.",
            metadata={
