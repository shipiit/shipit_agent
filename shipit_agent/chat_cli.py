...
def blue(t: str) -> str: return _ansi(t, "34")
...
PROVIDER_MODEL_ENV = {
    ...
    "together":  ("SHIPIT_TOGETHER_MODEL",  "together_ai/meta-llama/Llama-3.1-70B-Instruct-Turbo"),
...
AGENT_TYPES = ("agent", "deep", "goal", "reflective", "adaptive", "supervisor", "persistent")
    ...
    *,
    ...
    if agent_type == "supervisor":
        return Supervisor.with_builtins(
            llm=llm,
            worker_configs=[
                {"name": "researcher", "prompt": "You research information thoroughly."},
...
class ChatREPL:
    ...
    SLASH_COMMANDS = (
        "/help", "/agent", "/agents", "/tools", "/sources", "/index", "/rag",
        "/goal", "/reflect", "/verify", "/history", "/clear", "/save", "/load",
        "/reset", "/quiet", "/stream", "/provider", "/model", "/info",
        "/exit", "/quit",
    )
    ...
    def handle_command(self, line: str, *, output=print) -> int | None:
        ...
        if cmd in {"/exit", "/quit"}:
            ...
            return 0
        if cmd == "/help":
            self._print_help(output); return None
        if cmd == "/agent":
            self._switch_agent(arg, output); return None
        if cmd == "/agents":
            self._print_agent_types(output); return None
        if cmd == "/tools":
            self._print_tools(output); return None
        if cmd == "/sources":
            self._print_sources(output); return None
        if cmd == "/index":
            self._index_file(arg, output); return None
        if cmd == "/rag":
            self._print_rag(output); return None
        if cmd == "/goal":
            self._set_goal(arg, output); return None
        if cmd == "/reflect":
            self._toggle_reflect(arg, output); return None
        if cmd == "/verify":
            self._toggle_verify(arg, output); return None
        if cmd == "/history":
            self._print_history(output); return None
        if cmd == "/clear":
            self._clear_history(output); return None
        if cmd == "/save":
            self._save_history(arg, output); return None
        if cmd == "/load":
            self._load_history(arg, output); return None
        if cmd == "/reset":
            self._reset_session(output); return None
        if cmd == "/quiet":
            self.quiet = not self.quiet
            output(dim(f"[chat] event stream {'hidden' if self.quiet else 'visible'}")); return None
        if cmd == "/stream":
            self.typewriter_enabled = self._parse_bool(arg, self.typewriter_enabled)
            output(dim(f"[chat] typewriter streaming {'on' if self.typewriter_enabled else 'off'}")); return None
        if cmd == "/provider":
            self._switch_provider(arg, output); return None
        if cmd == "/model":
            self._switch_model(arg, output); return None
        if cmd == "/info":
            self._print_info(output); return None
    ...
    def _switch_provider(self, target: str, output) -> None:
        ...
        if not target:
            output(red(f"[chat] usage: /provider <{ '|'.join(PROVIDER_NAMES) }>")); return
        if target not in PROVIDER_MODEL_ENV:
            output(red(f"[chat] unknown provider: {target}. Options: {', '.join(PROVIDER_NAMES)}"))
            return
        ...
        output(dim(f"[chat] provider → {magenta(target)}  model → {cyan(current_model_name(target))}"))
    ...
    def _switch_model(self, target: str, output) -> None:
        ...
        env_var, _ = PROVIDER_MODEL_ENV.get(self.provider, ("", ""))
        if not env_var:
            output(red(f"[chat] provider {self.provider} has no configurable model env var"))
            return
    ...
    def handle_user_turn(self, user_text: str, *, output=print) -> None:
        try:
            ...
            try:
                for event in events:
                    ...
                    if etype == "run_completed":
                        payload_out = event.payload.get("output") if hasattr(event, "payload") else None
            ...
            if captured_sources:
                output(dim(f"  ({len(captured_sources)} RAG source(s) — /sources to see)"))
        except Exception as exc:
    ...
    def _banner(self, output) -> None:
        ...
        output(
            "  " + cyan("shipit chat --provider openai")
            + dim("   on launch")
        )
        output(
    ...
    def _print_help(self, output) -> None:
        ...
        for cmd, desc in rows:
            output(f"  {cyan(cmd):28s}  {dim(desc)}")
        output(dim("tip: type just ") + bold("/") + dim(" to open the command dropdown; press <TAB> to autocomplete."))
    ...
    def _switch_agent(self, target: str, output) -> None:
        target = target.strip().lower()
        if not target:
            output(red("[chat] usage: /agent <type>")); return
    ...
    def _print_sources(self, output) -> None:
        if not self.last_sources:
            output(dim("[chat] no RAG sources captured in the last turn")); return
    ...
    def _index_file(self, path: str, output) -> None:
        if not path:
            output(red("[chat] usage: /index <path>")); return
        if self.rag is None:
    ...
    def _print_rag(self, output) -> None:
        if self.rag is None:
            output(dim("[chat] no RAG attached")); return
        try:
    ...
    def _set_goal(self, objective: str, output) -> None:
        if not objective:
            output(red("[chat] usage: /goal <objective>")); return
    ...
    def _toggle_reflect(self, arg: str, output) -> None:
        if self.agent_type != "deep":
            output(yellow("[chat] /reflect only applies to --agent deep")); return
    ...
    def _toggle_verify(self, arg: str, output) -> None:
        if self.agent_type != "deep":
            output(yellow("[chat] /verify only applies to --agent deep")); return
    ...
    def _parse_bool(arg: str, default: bool) -> bool:
        s = arg.strip().lower()
        if s in {"on", "true", "1", "yes", "y"}: return True
        if s in {"off", "false", "0", "no", "n"}: return False
        return not default

    def _print_history(self, output) -> None:
        if self.chat is None:
            output(dim("[chat] this agent type does not expose history")); return
        history = self.chat.history()
        output(bold(f"{len(history)} message(s):"))
        for msg in history:
            colour = green if msg.role == "assistant" else cyan if msg.role == "user" else yellow
            output(f"  {colour(msg.role):20s} {msg.content[:200]}")

    def _clear_history(self, output) -> None:
        if self.chat is None:
            output(dim("[chat] no history to clear")); return
    ...
    def _save_history(self, path: str, output) -> None:
        if not path:
            output(red("[chat] usage: /save <path>")); return
        if self.chat is None:
            output(dim("[chat] this agent type has no chat history")); return
        history = self.chat.history()
        data = {
            "session_id": self.session_id,
    ...
    def _load_history(self, path: str, output) -> None:
        if not path:
            output(red("[chat] usage: /load <path>")); return
        if not os.path.exists(path):
            output(red(f"[chat] file not found: {path}")); return
        if self.chat is None:
            output(dim("[chat] current agent type has no chat history; switch to /agent agent or /agent deep first."))
            return
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--agent", default="deep", choices=list(AGENT_TYPES),
    ...
    parser.add_argument("--workspace", default=".shipit_workspace",
    ...
    parser.add_argument("--quiet", action="store_true",
                        help="Hide intermediate event stream")
    parser.add_argument("--no-stream", action="store_true",
                        help="Disable the typewriter streaming effect on the final answer")
    parser.add_argument("--rag-file", action="append", default=[],
                        help="Index a file (txt/md/pdf/html) into RAG before the session starts. Repeatable.")
    parser.add_argument("--rag-dim", type=int, default=512,
                        help="HashingEmbedder dimension when --rag-file is used (default 512)")
    parser.add_argument("--reflect", action="store_true",
                        help="Enable reflective mode (DeepAgent)")
    parser.add_argument("--verify", action="store_true",
                        help="Enable verification mode (DeepAgent)")
    parser.add_argument("--goal", default=None,
                        help="Goal objective (use with --agent goal)")
    ...
    if args.rag_file:
        ...
        for path in args.rag_file:
            try:
                ...
                sys.stderr.write(dim(f"[chat] indexed {len(chunks)} chunks from {path}\n"))
            except Exception as exc:
