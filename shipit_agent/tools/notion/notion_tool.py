...
class NotionTool(HTTPConnectorToolBase):
    ...
    def __init__(
        self,
        *,
        credential_key: str = "notion",
        credential_store=None,
        name: str = "notion",
        description: str = "Search Notion pages and create Notion pages.",
    ...
    def _headers(self, record):
        ...
        headers["Notion-Version"] = str(
            record.metadata.get("notion_version", "2022-06-28")
        )
        return headers
    ...
    def run(self, context: ToolContext, **kwargs: Any) -> ToolOutput:
        ...
        if action == "search_pages":
            data = self._request_json(
                ...
                method="POST",
                path="/v1/search",
                body={"query": str(kwargs.get("query", "")), "page_size": 10},
            )
            items = data.get("results", [])
            return ToolOutput(
                text="\n".join(item.get("url", "") for item in items)
                if items
                else "No Notion pages found.",
                metadata={
        ...
        if action == "create_page":
            body = {
                "parent": {"page_id": str(kwargs.get("parent_page_id", ""))},
                "properties": {
                    "title": {
                        "title": [{"text": {"content": str(kwargs.get("title", ""))}}]
                    }
                },
                "children": [
                    {
                        ...
                        "paragraph": {
                            "rich_text": [
                                {
                                    "type": "text",
                                    "text": {"content": str(kwargs.get("content", ""))},
                                }
                            ]
                        },
                    }
            ...
            data = self._request_json(
                record=record, method="POST", path="/v1/pages", body=body
            )
            return ToolOutput(
                text=f"Notion page created: {data.get('url', '')}",
                metadata={
                    "provider": self.provider,
                    "connected": True,
                    "action": action,
