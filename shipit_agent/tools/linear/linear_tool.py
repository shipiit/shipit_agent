...
class LinearTool(HTTPConnectorToolBase):
    ...
    def __init__(self, *, credential_key: str = "linear", credential_store=None, name: str = "linear", description: str = "Search Linear issues and create Linear issues.", prompt: str | None = None) -> None:
    ...
    def schema(self) -> dict[str, Any]:
        return {"type": "function", "function": {"name": self.name, "description": self.description, "parameters": {"type": "object", "properties": {"action": {"type": "string", "enum": ["search_issues", "create_issue", "get_issue", "update_issue", "list_teams", "list_projects"], "default": "search_issues"}, "query": {"type": "string"}, "title": {"type": "string"}, "team_id": {"type": "string"}, "project_id": {"type": "string"}, "issue_id": {"type": "string", "description": "Linear issue ID for update_issue"}, "identifier": {"type": "string", "description": "Linear issue identifier such as ENG-123 for get_issue"}, "description_text": {"type": "string"}, "state_id": {"type": "string"}, "assignee_id": {"type": "string"}, "priority": {"type": "integer"}, "limit": {"type": "integer", "default": 10}}, "required": ["action"]}}}
    ...
    def _graphql(self, *, record, query: str, variables: dict[str, Any]) -> dict[str, Any]:
        data = self._request_json(record=record, method="POST", path="", body={"query": query, "variables": variables})
        errors = data.get("errors", [])
        if errors:
            raise RuntimeError(f"Linear API error: {errors[0].get('message', 'unknown_error')}")
    ...
    def run(self, context: ToolContext, **kwargs: Any) -> ToolOutput:
        ...
        if action == "search_issues":
            data = self._graphql(
                ...
                variables={"term": str(kwargs.get("query", "")), "first": int(kwargs.get("limit", 10))},
            )
            items = data.get("issues", {}).get("nodes", [])
            return ToolOutput(text="\n".join(f"{item.get('identifier', '?')}: {item.get('title', '')} [{item.get('state', {}).get('name', '')}]" for item in items) if items else "No Linear issues found.", metadata={"provider": self.provider, "connected": True, "action": action, "items": items, "count": len(items)})
        if action == "create_issue":
            ...
            return ToolOutput(text=f"Linear issue created: {issue.get('identifier', '?')} {issue.get('title', '')}", metadata={"provider": self.provider, "connected": True, "action": action, "issue": issue})
        if action == "get_issue":
            identifier = str(kwargs.get("identifier", "")).strip()
            if not identifier:
                return ToolOutput(text="identifier is required for get_issue.", metadata={"provider": self.provider, "action": action})
            data = self._graphql(
                record=record,
            ...
            ) if issue else f"No Linear issue found for {identifier}."
            return ToolOutput(text=text, metadata={"provider": self.provider, "connected": True, "action": action, "issue": issue})
        if action == "update_issue":
            issue_id = str(kwargs.get("issue_id", "")).strip()
            if not issue_id:
                return ToolOutput(text="issue_id is required for update_issue.", metadata={"provider": self.provider, "action": action})
            data = self._graphql(
                record=record,
            ...
            return ToolOutput(text=f"Linear issue updated: {issue.get('identifier', issue_id)} {issue.get('title', '')}", metadata={"provider": self.provider, "connected": True, "action": action, "issue": issue})
        if action == "list_teams":
            ...
            return ToolOutput(text="\n".join(f"{item.get('name', '?')} ({item.get('key', '?')})" for item in items) if items else "No Linear teams found.", metadata={"provider": self.provider, "connected": True, "action": action, "items": items, "count": len(items)})
        if action == "list_projects":
            ...
            if query:
                items = [item for item in items if query in str(item.get("name", "")).lower() or query in str(item.get("slug", "")).lower()]
            return ToolOutput(text="\n".join(f"{item.get('name', '?')} ({item.get('slug', '?')}) [{item.get('state', '?')}]" for item in items) if items else "No Linear projects found.", metadata={"provider": self.provider, "connected": True, "action": action, "items": items, "count": len(items)})
